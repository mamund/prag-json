...
=== The +links+ Element
The +links+ element contains any links and/or forms associated with the *PRAG-JSON* document.  The +links+ collection is an array of anonymous JSON +link+ objects. These +link+ objects are designed to carry complete deatils on Web navigation between resources including any protocol methods, arguments, and so forth. Any time the message needs to render a navigation or state change, this should appear as a +link+ object in the +links+ collection. The +links+ object plays a role in *PRAG-JSON* similar to HTML5's +link+ (<<html5-link,HTML5-LINK>>), +A+ (<<html5-a, HTML5-A>>), and +form+ (<<html5-form, HTML5-FORM>>).
...
Below is a list of valid *PRAG-JSON* properties. These properties MAY appear in more than one place within a valid *PRAG-JSON* message. 
 ...
 * [[html5-tag]], [HTML5-TAG] Ian Hickson, "HTML5: Edition for Web Authors", 2011, https://www.w3.org/TR/2011/WD-html5-author-20110809/the-meta-element.html
